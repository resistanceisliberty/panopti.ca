...
export function DirectionField() {
  ...
  return (
    <div className="space-y-2">
      <label className="text-sm text-dark-200">{t('submit_direction_label')}</label>
      {directions.map((d, i) => (
      ...
      <button className="text-sm text-accent" onClick={() => patch({ directions: [...directions, ''] })}>{t('submit_add_direction')}</button>
    </div>
  );
}
